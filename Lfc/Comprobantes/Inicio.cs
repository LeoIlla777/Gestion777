...
namespace Lfc.Comprobantes
{
    public partial class Inicio : Lfc.FormularioListado
    {
        ...
        protected internal int m_Sucursal, m_FormaPago, m_Cliente, m_Vendedor, m_Anuladas = 1, m_PV = 0, m_Cobrador;
        ...
        public Inicio()
        {
            ...
            NombrePagina = NombrePagina == "" ? "ComprobanteConArticulos" : NombrePagina;
                ...
                Columns = new Lazaro.Pres.FieldCollection()
                    {
                        ...
                        new Lazaro.Pres.Field("comprob.fecha", "Fecha", Lfx.Data.InputFieldTypes.DateTime, 120),
                        ...
                        new Lazaro.Pres.Field("comprob.id_cobrador", "Cobrador", Lfx.Data.InputFieldTypes.Relation, 160),
                        new Lazaro.Pres.Field("comprob.obs", "Obs", Lfx.Data.InputFieldTypes.Memo, 160),
                        new Lazaro.Pres.Field("personas.id_persona", "PersonaID", Lfx.Data.InputFieldTypes.Integer, 0),
        ...
        protected override void OnBeginRefreshList()
        {
            ...
            if (m_Cobrador > 0)
                this.CustomFilters.AddWithValue("comprob.id_cobrador", m_Cobrador);
        ...
        protected override void OnItemAdded(ListViewItem item, Lfx.Data.Row row)
        {
            ...
            int IdCobrador = row.Fields["id_cobrador"].ValueInt;
            if (IdCobrador > 0)
            {
                Lfx.Data.Row Cobr = Lfx.Workspace.Master.Tables["personas"].FastRows[IdCobrador];
                if (Cobr != null)
                    item.SubItems["comprob.id_cobrador"].Text = Cobr.Fields["nombre_visible"].Value.ToString();
            }

            int IdComprob = row.Fields["id_comprob"].ValueInt;
            if (IdComprob > 0)
            {
                if (row.Fields["id_persona"].ValueInt == 998)
                {
                    string PersonaFree = this.Connection.FieldString(@"SELECT cliente_free FROM comprob_cliente WHERE id_comprob=" + IdComprob.ToString() + "");
                    if (PersonaFree != null && PersonaFree.Trim() != "")
                        item.SubItems["personas.nombre_visible"].Text = PersonaFree;
                }
            }
        }
    }
