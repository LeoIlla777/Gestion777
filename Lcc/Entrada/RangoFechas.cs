...
namespace Lcc.Entrada
{
    public partial class RangoFechas : Lui.Forms.Control
    {
        ...
        private bool m_FechaconHora = false;
        ...
        public bool MostrarFechaConHora {
            get {
                return m_FechaconHora;
            }
            set {
                m_FechaconHora = value;
                if (m_FechaconHora)
                    EntradaDesde.Width = EntradaHasta.Width = 140;
                else
                    EntradaDesde.Width = EntradaHasta.Width = 110;
            }
        }
        ...
        public Lfx.Types.DateRange Rango {
            ...
            set {
                ...
                if (MostrarFechaConHora)
                {
                    EntradaDesde.Text = Lfx.Types.Formatting.FormatDateAndTime(value.From);
                    EntradaHasta.Text = Lfx.Types.Formatting.FormatDateAndTime(value.To);
                }
                else
                {
        ...
        private void EntradaDesde_TextChanged(object sender, EventArgs e)
        {
            DbDateTime Fecha;
            if (m_FechaconHora)
                Fecha = Lfx.Types.Parsing.ParseSqlDateTime(EntradaDesde.Text);
            else
                Fecha = Lfx.Types.Parsing.ParseDate(EntradaDesde.Text);
            if (Fecha != null)
        ...
        private void EntradaHasta_TextChanged(object sender, EventArgs e)
        {
            DbDateTime Fecha;
            if (m_FechaconHora)
                Fecha = Lfx.Types.Parsing.ParseSqlDateTime(EntradaHasta.Text);
            else
                Fecha = Lfx.Types.Parsing.ParseDate(EntradaHasta.Text);
            if (Fecha != null)
