...
namespace Lcc.Edicion.Comprobantes
{
    public partial class Cobro : ControlEdicion
    {
        ...
        private bool sumaInteres = false, controlaCupon = true;
        private decimal totInt = 0;
        public decimal importeOriginal = 0;
        public Cobro()
        {
            InitializeComponent();
        }
        ...
        public Lfx.Types.OperationResult ValidateData()
        {
            ...
            switch (this.FormaDePago.Tipo)
            {
                ...
                case Lbl.Pagos.TiposFormasDePago.Tarjeta:
                    if (EntradaCupon.Text.Length == 0 && ControlCupon)
                        return new Lfx.Types.FailureOperationResult("Debe especificar el número de cupón.");
                    if (EntradaPlan.ValueInt <= 0 && SumaInteresEnTotal)
                        return new Lfx.Types.FailureOperationResult("Debe especificar el plan de la tarjeta.");
                    break;
        ...
        public bool SumaInteresEnTotal {
            get {
                return sumaInteres;
            }
            set {
        ...
        public bool ControlCupon {
            get {
                return controlaCupon;
            }
            set {
                controlaCupon = value;
            }
        ...
        private void OtroComando(object sender, EventArgs e)
        {
            using (Lcc.VerCuotas formCuotas = new VerCuotas())
            {
                formCuotas.Mostrar("Detalle de los montos en cuotas del siguiente importe: $" + EntradaImporte.ValueDecimal.ToString(), EntradaImporte.ValueDecimal);
                formCuotas.ShowDialog();
            }
        }

        private void EntradaInteres_TextChanged(object sender, EventArgs e)
        {
            if (SumaInteresEnTotal)
                SumarInteres();
        }

        public void SumarInteres()
        {
            EntradaImporte.ValueDecimal = importeOriginal * (1 + (EntradaInteres.ValueDecimal / 100));
            TotalInteres = EntradaInteres.ValueDecimal;
        }
    }

    public class ComandoBoton
    {
        public ComandoBoton()
        { }
        public virtual void LlamarVerCuota()
        {
