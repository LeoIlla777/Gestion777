...
namespace Lazaro.Base.Util.Impresion
{
    public class ImpresorElemento : Impresor
    {
        ...
        public override Lfx.Types.OperationResult Imprimir()
        {
            ...
            // Es una plantilla común... se imprime con Gestión
        ...
        {
            PaginaNumero = FilaActual = PaginaTotal = 0;
            Espejo = false;
            ...
            if (this.Plantilla != null)
            {
                ...
                switch (this.Plantilla.Tipo)
                {
                    ...
                    case Lbl.Impresion.TipoPlantilla.EnBlanco:
                        ...
                        if (Plantilla.TamanoPapel == "cont")
                            this.ImprimirInterna(e, e.PageBounds, false);
                        else
                            this.ImprimirInterna(e);
                        break;
        ...
        protected virtual void ImprimirInterna(PrintPageEventArgs e, Rectangle recorte, bool imprimirEspejos)
        {
            ...
            if (PaginaTotal == 0 || PaginaNumero <= PaginaTotal)
            ...
            foreach (Lbl.Impresion.Campo Cam in this.Plantilla.Campos)
                ...
                {
                    Espejo = imprimirEspejos;
                    this.ImprimirInterna(e, Cam.Rectangle, false);
                }

            if (PaginaTotal > 1 && !e.HasMorePages && PaginaNumero < PaginaTotal)//Se le saco el igual, y se dejó el menor.
                e.HasMorePages = true;
            else if (PaginaNumero >= PaginaTotal && e.HasMorePages)
        ...
        public virtual string ObtenerValorCampo(string nombreCampo, string formato)
        {
            switch (nombreCampo.ToUpperInvariant())
            {
                ...
                case "EMPRESA.CUIT":
                    return Lfx.Workspace.Master.CurrentConfig.Empresa.Cuit;
                case "EMPRESA.INICIOACTIVIDAD":
                case "EMPRESA.INICIOACT":
                    return Lfx.Workspace.Master.CurrentConfig.Empresa.InicioActividad;
                case "EMPRESA.RAZONSOCIAL":
                    return Lfx.Workspace.Master.CurrentConfig.Empresa.RazonSocial;
                case "EMPRESA.INGBRUTOS":
                case "EMPRESA.IB":
                    return Lfx.Workspace.Master.CurrentConfig.Empresa.IngresosBrutos;
                case "EMPRESA.IVA":
                    return Lfx.Workspace.Master.CurrentConfig.Empresa.IVA;
                default:
                    ...
                    if (Val != null && Val is DateTime || Val is DbDateTime)
                    {
                        if (Val is DbDateTime)
                            Val = ((DbDateTime)(Val)).Value;
                        if (formato != null)
                        {
                            try
                            {
