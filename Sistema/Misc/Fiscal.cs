    ...
    public partial class Fiscal : Lui.Forms.DialogForm
    {
        ...
        private void BotonIniciarDetener_Click(object sender, System.EventArgs e)
        {
            if (BotonIniciarDetener.Text == "Iniciar")
            {
                ...
                {
                    // Servidor remoto
                    Lfx.Workspace.Master.DefaultScheduler.AddTask("FISCAL INICIAR", "gestion777", EstacionFiscal);
                    Lui.Forms.MessageBox.Show("Se envió una orden de iniciar el servidor fiscal a través de la red. Para que esto funcione es necesario que Gestión esté funcionando en el equipo remoto.", "Iniciar remoto");
                }
            }
            else
            {
        ...
        private void BotonCierreX_Click(object sender, EventArgs e)
        {
            Lfx.Workspace.Master.DefaultScheduler.AddTask("CIERRE X", "fiscal" + this.Pventa.ToString(), "*");
            Lui.Forms.MessageBox.Show("Se envió un comando de Cierre X al punto de venta seleccionado.", "Cierre X");
            BotonCierreX.Enabled = false;
        }
    }
}