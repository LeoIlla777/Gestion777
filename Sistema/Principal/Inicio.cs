...
namespace Lazaro.WinMain.Principal
{
    public partial class Inicio : Form
    {
        ...
        private void FormPrincipal_Load(object sender, EventArgs e)
        {
            ...
            string nombreApp = "Excelencia Gestión";
            ...
            {
                case "normal":
                    this.Text = nombreApp + " - " + Lbl.Sys.Config.Actual.UsuarioConectado.Persona.Nombres + " en " + Lfx.Workspace.Master.ToString();
                    break;
                case "maximizado":
            ...
            if (Lfx.Workspace.Master.CurrentConfig.ReadGlobalSetting<int>("Sistema.Apariencia.NoMostrarInicio", 0) == 0)
            {
                ...
                FormInicio.Show();
                MostrarAyuda("Bienvenido a " + nombreApp, "Pulse la tecla <F12> para mostrar la pantalla de inicio.");
            }
            else
            {
                MostrarAyuda("...Bienvenido a " + nombreApp, "Pulse la tecla <F12> para activar el menú principal.");
            }
        }
            ...
            if (this.Visible)
            {
                ...
                if (Lfx.Workspace.Master.SlowLink || Lfx.Environment.SystemInformation.DesignMode)
                {
                    if (Lfx.Workspace.Master.DefaultScheduler.LastGetTask == System.DateTime.MinValue || (DateTime.Now - Lfx.Workspace.Master.DefaultScheduler.LastGetTask).Minutes >= 1)
                        ProximaTarea = Lfx.Workspace.Master.DefaultScheduler.GetNextTask("Gestion");
                }
                else
                {
                ...
                if (YaPregunteReiniciar == false && Lfx.Updates.Updater.Master != null && Lfx.Updates.Updater.Master.UpdatesPending() && ActiveForm == this)
                {
                    ...
                    Lui.Forms.YesNoDialog Pregunta = new Lui.Forms.YesNoDialog("Existe una nueva versión de Gestión. Debe reiniciar la aplicación para instalar la actualización.", "¿Desea reiniciar ahora?");
        ...
        private void FormPrincipal_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                ...
                case Keys.T:
                    ...
                    {
                        e.Handled = true;
                        Ejecutor.Exec("CREAR Lbl.Comprobantes.TicketX");
                    }
                    break;
                case Keys.P:
                    if (e.Control == true && e.Alt == false && e.Shift == false)
        ...
        public void MostrarPaginas(object form)
        {
            if (BarraInferior.Visible)
                BarraInferior.MostrarPaginas(form);
        }

        public void ActualizarPaginas(int pag, int totPag)
        {
            if (BarraInferior.Visible)
                BarraInferior.ActualizarPaginas(pag, totPag);
        }
        ...
        private void CargarMenuXml(XmlNode node, Menu colgarDe, string parentText)
        {
            ...
            if (node.ChildNodes.Count > 0)
            {
                foreach (XmlNode opcion in node.ChildNodes)
                {
                    ...
                    if (opcion.Attributes["Imagen"] != null)
                        Itm.Text += opcion.Attributes["Imagen"].Value;
        ...
        private void IconosMenu(DrawItemEventArgs e, string nombre)
        {
            switch (nombre)
            {
                case "archivos16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.archivos16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "calendario16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.calendario16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "cheque16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.cheque16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "cheques16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.cheques16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "commerce16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.commerce16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "credito16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.credito16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "dolar16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.dolar16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "gears":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.gears16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "impresora16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.impresora16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "key16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.key16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "laptop16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.laptop16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "money16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.money16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "persona16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.persona16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "qrcode16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.qrcode16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "salir":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.stop, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "tarjeta16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.tarjeta16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "tarjetaid16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.tarjetaid16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
                case "calculadora16":
                    e.Graphics.DrawImage(Lazaro.Properties.Resources.calculadora16, e.Bounds.X, e.Bounds.Y + 5, 16, 16);
                    break;
            }
        }
